...
def process_video(video_path: str, zones: List[Zone], lines: List[Line], output_path: str, progress_id: str = None) -> Dict:
    """Process video with ByteTrack tracking"""
...
async def process(request: ProcessRequest):
    ...
    try:
        stats = process_video(str(input_path), request.zones, request.lines, str(output_path), progress_id)
        
        # Clean up progress tracking after completion
        if progress_id in processing_progress:
            del processing_progress[progress_id]
...
async def get_video_url(filename: str):
    source_path = OUTPUT_FOLDER / filename
    ...
    simple_filename = f"video_{file_hash}.mp4"

    dest_path = STATIC_FOLDER / simple_filename

    if not dest_path.exists():
        shutil.copy2(source_path, dest_path)

    video_url = f"/static/{simple_filename}"
...
async def get_history():
    ...
    # Look for both processed_* and temp_processed_* files
    video_patterns = ['processed_*.mp4', 'temp_processed_*.mp4']
    found_files = set()
    
    for pattern in video_patterns:
        for video_file in OUTPUT_FOLDER.glob(pattern):
            if video_file.name in found_files:
                continue
            found_files.add(video_file.name)
            
            video_name = video_file.name
            # Handle both naming conventions
            base_name = video_name.replace('temp_processed_', '').replace('processed_', '').rsplit('.', 1)[0]
...
if __name__ == '__main__':
    import uvicorn
    print("\n" + "="*60)
    print("🚀 Person Detection & Tracking Application")
    print("="*60)
